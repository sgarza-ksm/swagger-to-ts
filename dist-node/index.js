...
function camelCase$1(name) {
  return name.replace(/(-|_|\.|\s)+\w/g, letter => letter.toUpperCase().replace(/[^0-9a-z]/gi, ''));
}

function pascal(name) {
  return camelCase$1(name).replace(/[a-z]/, letter => letter.toUpperCase());
}
...
function sanitize$1(name) {
  return name.includes('-') ? `'${name}'` : name;
}

function parse$1(spec, options = {}) {
  const shouldUseWrapper = options.wrapper !== false;
  const wrapper = typeof options.wrapper === 'string' && options.wrapper ? options.wrapper : 'declare namespace OpenAPI3';
  ...
  function getType(schema, nestedName) {
    const {
      ...
      nullable
          ...
          value = _objectWithoutProperties(schema, ["$ref", "items", "type", "nullable"]);

    const nextInterface = camelCase$1(nestedName);
    const DEFAULT_TYPE = 'any';
    ...
    if ($ref) {
      ...
      if (refProperties.type && TYPES$1[refProperties.type]) {
    ...
    if (items && items.type) {
      ...
      if (TYPES$1[items.type]) {
        return `${TYPES$1[items.type]}[]`;
      }
    ...
    if (type) {
      return TYPES$1[type] || type || DEFAULT_TYPE;
    }
  ...
  function buildNextInterface() {
    ...
    if (!Object.keys(allProperties).length && additionalProperties !== true && type && TYPES$1[type]) {
      return;
    }
